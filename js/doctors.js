...
Doctor.prototype.searchBySymptom = function(condition, displayDoctors, displayError) {
  $.get('https://api.betterdoctor.com/2016-03-01/doctors?query=' + condition + '&location=45.523%2C-122.677%2C1000&user_location=45.523%2C-122.677&sort=distance-asc&skip=0&limit=25&user_key=' + apiKey).then(function(response) {
...
Doctor.prototype.searchBySpecialty = function(specialty, displayDoctors, displayError) {
  $.get('https://api.betterdoctor.com/2016-03-01/doctors?specialty_uid=' + specialty + '&location=45.523%2C-122.677%2C100&user_location=45.523%2C-122.677&sort=distance-asc&skip=0&limit=25&user_key=' + apiKey).then(function(response) {
    if (response.data.length > 0) {
      for (var i = 0; i < response.data.length; i++) {
        var doctorFirstName = response.data[i].profile.first_name;
        var doctorLastName = response.data[i].profile.last_name;
